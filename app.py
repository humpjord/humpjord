...
# Cities too generic to display in the neighborhood box
...
def is_address(text):
    ...
    if not text:
        return False
    # Starts with a digit (e.g. "28927 W. Beach Lane")
    if text[0].isdigit():
        return True
...
def clean_neighborhood(text):
    """Strip state/zip suffix, reject street addresses and generic city names."""
    if not text:
        return ""
    if is_address(text):
        return ""
    # Strip trailing ", CA 90265" or "CA 90265" style suffix
...
def build_image(photo_url, beds, baths, price, neighborhood, description):
    ...
    # Fix white vertical line: extend dark description rect to x=0 and remove stroke
    svg = svg.replace(
    ...
    # Center dollar sign icon — shift right and down to center in circle
    svg = re.sub(
        r'(<image width="379" height="601" transform=")translate\(516\.35 540\.97\) scale\(\.13\)',
        r'\1translate(520.35 543.97) scale(.13)',
        svg
    )
    ...
    # Description: clean boilerplate, cap at 4 lines
    description = clean_description(description)
    lines = textwrap.wrap(description, width=44)[:4]
    tspans = "".join(
    ...
    # Footer: visible within Instagram's 4:5 feed crop (output y < ~1350px)
    # Description 4 lines ends at SVG y≈1240; footer sits below with clearance.
    footer = (
        '<text x="542" y="1290" text-anchor="middle" fill="#fff" '
        'font-family="Cosmata-ExtraBold, Cosmata" font-size="26" '
        'letter-spacing="3" font-weight="700">EMAIL US FOR DETAILS</text>'
        '<text x="542" y="1330" text-anchor="middle" fill="#fff" '
        'font-family="Cosmata-ExtraBold, Cosmata" font-size="26" '
        'letter-spacing="3" font-weight="700">LINK IN BIO</text>'
    )
...
def generate():
    ...
    try:
        filename = build_image(
            ...
            str(data["baths"]),
            str(data["price"]),
            clean_neighborhood(str(data["neighborhood"]).strip()).upper(),
