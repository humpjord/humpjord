...
def build_image(photo_url, beds, baths, price, neighborhood, description):
    ...
    # Fix "OFF-MARKET LOS ANGELES": replace the full element with direct x/y,
    # text-anchor="middle", and reduced font-size so it fits the blue box.
    svg = re.sub(
    ...
    # Neighborhood: replace full element with direct x/y and text-anchor="middle"
    ...
    # Description: clean boilerplate, cap at 5 lines so footer is visible
    # in Instagram's 4:5 feed crop (1080x1350).
    description = clean_description(description)
    lines = textwrap.wrap(description, width=44)[:5]
    tspans = "".join(
    ...
    # Footer: "EMAIL US FOR DETAILS / LINK IN BIO" — placed within the
    # 4:5 Instagram crop zone (y < ~1350 SVG units).
    footer = (
        '<text x="542" y="1305" text-anchor="middle" fill="#fff" '
        'font-family="Cosmata-ExtraBold, Cosmata" font-size="28" '
        'letter-spacing="3" font-weight="700">EMAIL US FOR DETAILS</text>'
        '<text x="542" y="1340" text-anchor="middle" fill="#fff" '
        'font-family="Cosmata-ExtraBold, Cosmata" font-size="28" '
