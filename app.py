...
def is_address(text):
    """Return True if text looks like a street address rather than a neighborhood name."""
    if not text:
        return False
...
def clean_neighborhood(text):
    """Strip state/zip suffix and reject street addresses."""
    if is_address(text):
        return ""
    # Strip ", CA 90265" or similar trailing state+zip
...
def build_image(photo_url, beds, baths, price, neighborhood, description):
    ...
    # Note: "EMAIL US FOR DETAILS" and "LINK IN BIO" are already built into
    # the SVG template as vector paths at the bottom (y≈1694 and y≈1768).
